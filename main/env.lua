...
function sudo.print(s) Trace('KITSU: '..(s or 'nil')) end
function sudo.printerr(s) SCREENMAN:SystemMessage('KITSU: '..(s or 'nil')) end

-- Library importer
function sudo.import(lib)
	-- Catch in case we add .lua to our path.
	if string.find(path, '%.lua') then lib = string.sub(lib, 1, string.find(lib, '%.lua') - 1) end
	-- Make sure the file is there
	local file = 'lib/'..lib..'.lua'
...
function sudo.run(path)
	-- Catch in case we add .lua to our path.
	if string.find(path, '%.lua') then path = string.sub(path, 1, string.find(path, '%.lua') - 1) end
	-- Make sure the file is there
	local file = path..'.lua'
	if not assert(loadfile(file)) then
